...
(function (global) {
  ...
  function normalizePort(port) {
    ...
    const value = typeof port === "number" ? port : Number.parseInt(String(port).trim(), 10);
  ...
  function matchesIpFilter(ipStr, filters) {
    ...
    return filters.some(
      (filter) =>
        (filter.type === "exact" && num === filter.value) ||
        (filter.type === "cidr" && (num & filter.mask) === filter.base) ||
        (filter.type === "range" && num >= filter.start && num <= filter.end)
    );
  }
  ...
  function matchesEndpointFilter(tokens, name, ip) {
    ...
    return tokens.some((token) => {
      ...
      return nameLc.includes(token) || ipLc.includes(token);
    });
  }
  ...
  function matchTokens(text, tokens) {
    ...
    if (excludes.length && excludes.some((t) => lower.includes(String(t.value || "").toLowerCase()))) {
      return false;
    }
  ...
  function edgeMatches(data, srcTokens, dstTokens, portMatcher, excludePublic, excludeNoisePorts, ipFilters) {
    ...
    if (excludeNoisePorts && isDefaultHiddenPort(port)) {
      return false;
    }

    if (portMatcher && !portMatcher(port)) {
      return false;
    }

    if (excludePublic && data.isPublic) {
      return false;
    }

    if (ipFilters && ipFilters.length) {
      if (srcIp && matchesIpFilter(srcIp, ipFilters)) {
        return false;
      }

      if (dstIp && matchesIpFilter(dstIp, ipFilters)) {
        return false;
      }
    }
